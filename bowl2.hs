...
score' n (Strike:_)                = [10] -- error "incomplete frame for strike (1)"
...
score' n (Spare _ _:_)             = [10] -- error "incomplete frame for spare"
...
disp Strike         = "  X"
disp (Incomplete a) = (show a) ++ " (incomplete)"
...
foo2 :: [Int] -> IO ()
foo2 xs = do putStrLn $ unlines $ map disp2 ts
             putStrLn $ "Total: " ++ (show total)
             putStrLn ""
  where ts = foo xs
        total = sum $ catMaybes $ map snd ts
